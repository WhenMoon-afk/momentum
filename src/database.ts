...
export class MomentumDatabase {
  ...
  healthCheck(): { ok: boolean; details: Record<string, unknown> } {
    try {
      ...
      `).get() as { session_count: number; snapshot_count: number; total_tokens: number };
   ...
   */
  private formatContext(context: string | StructuredContext): string {
  ...
  getSnapshotById(id: number): Snapshot | undefined {
    const result = this.db.prepare('SELECT * FROM snapshots WHERE id = ?').get(id) as Snapshot | null;
   ...
   */
  getCompactedContext(sessionId?: string, maxTokens: number = 15000): CompactResult {
   ...
   */
  private formatSnapshotForCompact(snapshot: Snapshot, isNewest: boolean = false): string {
  ...
  getContextForInjection(
    sessionId?: string,
    options: { topic?: string; includeCritical?: boolean; maxTokens?: number } = {}
  ): CompactResult {
      ...
      snapshots = sessionId
        ...
        : this.db.prepare(`
            ...
            ORDER BY
              CASE importance WHEN 'critical' THEN 4 WHEN 'important' THEN 3 ELSE 2 END DESC,
              created_at DESC
          `).all(searchPattern, searchPattern) as Snapshot[];
   ...
   */
  private formatSnapshotForInjection(snapshot: Snapshot): string {
   ...
   */
  getUnsyncedSnapshots(limit: number = 100): Array<{ snapshot: Snapshot; projectPath: string | null }> {
    ...
    const results = this.db.prepare(`
      ...
      LIMIT ?
    `).all(safeLimit) as Array<Snapshot & { project_path: string | null }>;
  ...
  markSynced(snapshotId: number, cloudId?: string): void {
    this.db.prepare(
      'UPDATE snapshots SET synced = 1, cloud_id = ? WHERE id = ?'
    ).run(cloudId || null, snapshotId);
  }
  ...
  markMultipleSynced(snapshotIds: number[]): void {
    ...
    this.db.prepare(
      `UPDATE snapshots SET synced = 1 WHERE id IN (${placeholders})`
    ).run(...snapshotIds);
  }
  ...
  getUnsyncedCount(): number {
    const result = this.db.prepare(
      'SELECT COUNT(*) as count FROM snapshots WHERE synced = 0 OR synced IS NULL'
    ).get() as { count: number };
    return result.count;
  }
   ...
   */
  getRetryableSnapshots(limit: number = 10): Array<{ snapshot: Snapshot; projectPath: string | null; retries: number }> {
    const safeLimit = Math.min(Math.max(1, limit), 100);
    const maxRetries = 5;
    ...
    const results = this.db.prepare(`
      ...
      LIMIT ?
    `).all(maxRetries, safeLimit) as Array<Snapshot & { project_path: string | null; retries: number }>;
  ...
  getFailedSyncCount(): number {
    ...
    const result = this.db.prepare(`
      ...
      WHERE (synced = 0 OR synced IS NULL)
        AND sync_retries >= ?
    `).get(maxRetries) as { count: number };
    return result.count;
  }
