...
function getDbPath(): string {
  if (process.env.SNAPSHOT_DB_PATH) return process.env.SNAPSHOT_DB_PATH;
...
class MomentumServer {
      ...
      try {
        switch (name) {
          ...
          case 'import_snapshots':
            return await this.handleImportSnapshots(args as unknown as { db_path?: string });
          default:
  ...
  private async handleLoadSnapshot(args: { id?: number; name?: string }) {
    ...
    if (args.id !== undefined) {
      ...
      if (!snapshot) throw new Error(`Snapshot with ID ${args.id} not found`);
    } else if (args.name) {
      snapshot = this.db.getSnapshotByName(args.name);
      if (!snapshot) throw new Error(`Snapshot with name "${args.name}" not found`);
    } else {
  ...
  private async handleDeleteSnapshot(args: { id: number }) {
    ...
    if (!deleted) throw new Error(`Snapshot with ID ${args.id} not found`);
  ...
  private async handleImportSnapshots(args: { db_path?: string }) {
    const dbPath = args.db_path || process.env.SNAPSHOT_DESKTOP_DB;

    if (!dbPath) {
      throw new Error(
        'No database path provided. Pass db_path or set SNAPSHOT_DESKTOP_DB environment variable.'
      );
    }
